...
export default function TwitterForm({imageUrl, userName, handle, post }: TwitterFormProps) {

  const [ isSaving, setIsSaving] = useState(false)

  const defaultValues: Partial<TwitterFormValues> = {
    tweets: post.content && post.content.length > 0 ? post.content.map((text) => ({text: text})) : [{text: ""}]
  }
